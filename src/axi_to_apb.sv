...
// Description: AXI4 to APB4 bridge
//
// Bridges one AXI4 slave port onto one or more APB4 slaves, supporting data-width downsizing
...
//
// Behaviour notes:
//   - ATOPs are not supported (by this module and also APB), and are filtered out with an `axi_atop_filter`.
...
module axi_to_apb #(
  ...
  ///`NoRules == `0` is only legal with `NoApbSlaves == 1`.
  ...
  // APB slave address map (unused when `NoRules == 0`)
  input  rule_t     [((NoRules > 0) ? NoRules : 1)-1:0] addr_map_i
);

  // Build the address decoder only when rules are provided; otherwise forward everything to slave 0.
  localparam bit UseAddrDecode = (NoRules > 32'd0);
  ...
  // Address decode on the selected lane's (full-width) address. With no rules (`NoRules == 0`) the
  // decoder is bypassed: the lone APB slave is always selected and every address is a hit.
  logic     apb_dec_valid;
  sel_idx_t apb_sel_idx;
  if (UseAddrDecode) begin : gen_addr_decode
    addr_decode #(
  ...
  end else begin : gen_no_decode
    assign apb_sel_idx   = '0;   // only slave 0 exists
    assign apb_dec_valid = 1'b1; // every transaction is a hit
  end
  ...
  initial begin : check_params
    ...
    apb_no_decode: assert (UseAddrDecode || NoApbSlaves == 32'd1) else
      $fatal(1, $sformatf("NoRules == 0 (no address decoder) is only allowed with a single APB slave"));
  end
  `endif
  // pragma translate_on
endmodule
...
module axi_to_apb_intf #(
  ...
  // APB slave address map (unused when `NoRules == 0`; port width kept at >=1 to stay well-formed)
  input  rule_t [((NoRules > 0) ? NoRules : 1)-1:0] addr_map_i
);
